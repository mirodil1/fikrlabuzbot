...
async def process_education(message: Message, state: FSMContext):
    ...
    await message.answer(
        "<b>Ish joyingizni kiriting.</b>\n"
        "<i>(Namuna: O'qituvchi, 123-maktab)</i>",
    )


@submission_router.message(SubmissionState.profession, F.text)
async def process_profession(message: Message, bot: Bot, state: FSMContext):
